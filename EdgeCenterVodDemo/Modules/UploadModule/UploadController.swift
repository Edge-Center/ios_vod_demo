...
final class UploadController: BaseViewController {
    ...
    override func errorHandle(_ error: Error) {
        guard let error = error as? ErrorResponse else {     
            let alert = createAlert(title: ErrorResponse.unexpectedError.description)
        ...
        case .invalidCredentials:
            ...
            let alert = createAlert(title: error.description, actionHandler: actionHandler)
            present(alert, animated: true)
...
//MARK: - extensions ECCameraViewDelegate, ECCameraDelegate
